...
<script>
  export default {
    ...
    methods: {
      ...
      loadTreeData(){
        var _this = this;
        this.getRequest("sso-service", "/system/basic/dep/-1").then(resp=> {
      ...
      addDep(){
        ...
        this.treeLoading = true;
        this.postRequest("sso-service", "/system/basic/dep", {
          name: this.depName,
          parentId: this.pDep
        }).then(resp=> {
          _this.treeLoading = false;
      ...
      loadAllDeps(){
        var _this = this;
        this.getRequest("sso-service", "/system/basic/deps").then(resp=> {
          if (resp && resp.status == 200) {
      ...
      deleteDep(data, event){
        ...
        if (data.children.length>0) {
          ...
          this.$confirm('删除[' + data.name + ']部门, 是否继续?', '提示', {
            ...
            _this.deleteRequest("sso-service", "/system/basic/dep/" + data.id).then(resp=> {
              _this.treeLoading = false;
              if (resp && resp.status == 200) {
                var respData = resp.data;
                _this.$message({
