    ...
    methods: {
      deleteRole(rid, rname){
        ...
        this.$confirm('删除角色[' + rname + '], 是否继续?', '提示', {
          ...
          _this.loading = true;
          _this.deleteRequest("sso-service", "/system/basic/role/" + rid).then(resp=> {
      ...
      addNewRole(){
        if (isNotNullORBlank(this.newRole, this.newRoleZh)) {
          this.loading = true;
          var _this = this;
          this.postRequest("sso-service", "/system/basic/addRole", {
            role: this.newRole,
      ...
      updateRoleMenu(index){
        ...
        var _this = this;
        this.putRequest("sso-service", "/system/basic/updateMenuRole", {
      ...
      collapseChange(activeName){
        ...
        var _this = this;
        this.getRequest("sso-service", "/system/basic/menuTree/" + activeName).then(resp=> {
          if (resp && resp.status == 200) {
            var data = resp.data;
...
//        console.log(data,checked,indeterminate)
      ...
      initRoles(){
        var _this = this;
        this.getRequest("sso-service", "/system/basic/roles").then(resp=> {
          _this.loading = false;
          if (resp && resp.status == 200) {
